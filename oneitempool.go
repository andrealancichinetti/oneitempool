...
You have a hot-spot function that requires some memory allocations.
Instead of allocating the memory each time you call the function, you can use
sync.Pool, right?

Yes, but you get some overhead. Also, you might not want to lose the object
if the GC runs.
...
this simple struct will make sure that your code will only get access to the data only once
